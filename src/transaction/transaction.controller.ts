...
export class TransactionController {
  ...
  async transfer(@Body() dto: TransferDto): Promise<ApiResponseInterface<{
    transaction: Transaction;
    fromWallet: { id: string; balance: number };
  ...
  async transferAsync(@Body() dto: TransferDto): Promise<ApiResponseInterface<{ message: string }>> {
    await this.transactionService.processTransferAsync(dto);
    return {
      status: true,
  ...
  @ApiQuery({ name: 'type', description: 'Transaction type filter', required: false })
  ...
  async getTransactionHistory(@Request() req: any): Promise<ApiResponseInterface<{
    ...
    const { walletId, page = 1, limit = 20, type, status, startDate, endDate } = req.query;
    ...
    const result = await this.transactionService.getTransactionHistory(query);
    return {
      status: true,
  ...
  async getTransaction(@Request() req: any): Promise<ApiResponseInterface<Transaction>> {
    const result = await this.transactionService.getTransaction(req.params.transactionId);
    return {
      status: true,
  ...
  async getTransactionStats(@Request() req: any): Promise<ApiResponseInterface<{
    totalDeposits: number;
    totalWithdrawals: number;
    totalTransfers: number;
    totalFees: number;
  }>> {
    const result = await this.transactionService.getTransactionStats(req.params.walletId);
    return {
      status: true,
  ...
  @ApiQuery({ name: 'status', description: 'Transaction status', required: false })
  ...
  async getTransactionsByFilter(@Request() req: any): Promise<ApiResponseInterface<any>> {
    const { 
      walletId, type, status, startDate, endDate, minAmount, maxAmount, 
      page = 1, limit = 20 
    } = req.query;
  ...
  async getFailedTransactions(@Request() req: any): Promise<ApiResponseInterface<any>> {
  ...
  async getPendingTransactions(@Request() req: any): Promise<ApiResponseInterface<any>> {
    const { page = 1, limit = 20 } = req.query;
  ...
  @ApiQuery({ name: 'limit', description: 'Items per page', required: false })
  @ApiResponse({ status: 200, description: 'Transactions retrieved successfully' })
  async getTransactionsByDateRange(@Request() req: any): Promise<ApiResponseInterface<any>> {
